...
export default function LoginPage() {
  ...
  return (
    <main className="mx-auto flex max-w-xl flex-col items-center px-6 py-14">
      ...
      <div className="mt-8 grid w-full grid-cols-2 gap-2 rounded-2xl border border-slate-200 bg-slate-100 p-1.5">
        <button
          onClick={() => {
            setRole("citizen");
            setError(null);
      ...
      <form onSubmit={handleSubmit} className="mt-8 w-full rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <h2 className="text-3xl font-bold text-nagar-ink">{role === "citizen" ? "Citizen Login" : "Authority Login"}</h2>
        <p className="mb-6 mt-1 text-lg text-slate-500">
          {role === "citizen" ? "Sign in to file complaints and track issues." : "Manage grievance workflow and closures."}
        </p>
        ...
        <Input
          name="email"
          label="Email"
          placeholder={role === "authority" ? "admin@nagarseva.gov" : "citizen@nagarseva.com"}
          type="email"
        />
